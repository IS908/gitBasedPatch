...
cd ${ZIP_HOME}/target 
unzip -q ${SOURCE}
mv ${UNZIP_NAME} ${APP_NAME}
echo App_${TAG_NAME} > ${ZIP_HOME}/target/${APP_NAME}/versionid.txt
echo App_${TAG_NAME} > ${ZIP_HOME}/target/${APP_NAME}/version_list.txt
...
echo "移动应用到部署目录"
mv ${ZIP_HOME}/target/${APP_NAME} ${DCITS_HOME}
